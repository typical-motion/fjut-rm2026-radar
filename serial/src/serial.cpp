#include <thread>
#include <iostream>
#include <termios.h>
#include <unistd.h>
#include <fcntl.h>
#include <serial/serial.h>
#include <chrono>
#include <map>
#include <string>
#include <vector>
#include <cstdint>
#include <unordered_map>

#include <algorithm> // for std::find
#include <mutex>     // for std::mutex
#include <tuple>     // for std::tuple
#include <typeinfo>  // for typeid
#include "serial.hpp"
using namespace std;
using namespace serial;


//struct RadarDecision
//{
//    uint8_t double_vulnerability_chance;
//    uint8_t opponent_double_vulnerability;
//    uint8_t reserved_bits;
//};


int double_vulnerability_chance = -1; //双倍易伤机会次数
int opponent_double_vulnerability = -1; // 是否正在触发双倍易伤
int chances_flag = 1; // 双倍易伤触发标志位，需要从1递增，每小局比赛会重置，所以每局比赛要重启程序
//int progress_list[6]= {0,0,0,0,0,0};
std::vector<int> progress_list; //标记进度列表

map<std::string,int> mark_value = 
    {
        {"B1",0},
        {"B2",0},
        {"B7",0},
        {"R1",0},
        {"R2",0},
        {"R7",0}
    };

    //机器人名字对应ID
    map<std::string,int> mapping_table =
    {
        {"R1", 1},
        {"R2", 2},
        {"R3", 3},
        {"R4", 4},
        {"R5", 5},
        {"R6", 6},
        {"R7", 7},
        {"B1", 101},
        {"B2", 102},
        {"B3", 103},
        {"B4", 104},
        {"B5", 105},
        {"B6", 106},
        {"B7", 107}
    }; 

    // CRC8校验表：G(x)=x8+x5+x4+1（改全局
    const uint8_t CRC8_INIT = 0xff;
    const uint8_t CRC8_TAB[500] = 
    {
        
    };

    // CRC16校验表（改全局
    const uint16_t CRC_INIT = 0xffff;
    const uint16_t wCRC_Table[256] = 
    {
        0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf,
        0x8c48, 0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7,
        0x1081, 0x0108, 0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e,
        0x9cc9, 0x8d40, 0xbfdb, 0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876,
        0x2102, 0x308b, 0x0210, 0x1399, 0x6726, 0x76af, 0x4434, 0x55bd,
        0xad4a, 0xbcc3, 0x8e58, 0x9fd1, 0xeb6e, 0xfae7, 0xc87c, 0xd9f5,
        0x3183, 0x200a, 0x1291, 0x0318, 0x77a7, 0x662e, 0x54b5, 0x453c,
        0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd, 0xc974,
        0x4204, 0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb,
        0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1, 0xab7a, 0xbaf3,
        0x5285, 0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a,
        0xef4e, 0xfec7, 0xcc5c, 0xddd5, 0xa96a, 0xb8e3, 0x8a78, 0x9bf1,
        0x7387, 0x620e, 0x5095, 0x411c, 0x35a3, 0x242a, 0x16b1, 0x0738,
        0xffcf, 0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862, 0x9af9, 0x8b70,
        0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5, 0xe13e, 0xf0b7,
        0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff,
        0x9489, 0x8500, 0xb79b, 0xa612, 0xd2ad, 0xc324, 0xf1bf, 0xe036,
        0x18c1, 0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e,
        0xa50a, 0xb483, 0x8618, 0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5,
        0x2942, 0x38cb, 0x0a50, 0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd,
        0xb58b, 0xa402, 0x9699, 0x8710, 0xf3af, 0xe226, 0xd0bd, 0xc134,
        0x39c3, 0x284a, 0x1ad1, 0x0b58, 0x7fe7, 0x6e6e, 0x5cf5, 0x4d7c,
        0xc60c, 0xd785, 0xe51e, 0xf497, 0x8028, 0x91a1, 0xa33a, 0xb2b3,
        0x4a44, 0x5bcd, 0x6956, 0x78df, 0x0c60, 0x1de9, 0x2f72, 0x3efb,
        0xd68d, 0xc704, 0xf59f, 0xe416, 0x90a9, 0x8120, 0xb3bb, 0xa232,
        0x5ac5, 0x4b4c, 0x79d7, 0x685e, 0x1ce1, 0x0d68, 0x3ff3, 0x2e7a,
        0xe70e, 0xf687, 0xc41c, 0xd595, 0xa12a, 0xb0a3, 0x8238, 0x93b1,
        0x6b46, 0x7acf, 0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9,
        0xf78f, 0xe606, 0xd49d, 0xc514, 0xb1ab, 0xa022, 0x92b9, 0x8330,
        0x7bc7, 0x6a4e, 0x58d5, 0x495c, 0x3de3, 0x2c6a, 0x1ef1, 0x0f78
    };

//class SerialPort
//{
    //public:
    //CRC8校验
    uint8_t Get_CRC8_Check_Sum(const std::vector<uint8_t>& pchMessage,size_t dwLength)
    {
        uint8_t ucCRC8 = CRC8_INIT;
        for(size_t i = 0; i < dwLength; i++)
        {
            ucCRC8 = CRC8_TAB[ucCRC8 ^ pchMessage[i]];
        }
        return ucCRC8;
    }
    // CRC16校验
    uint16_t Get_CRC16_Check_Sum(const std::vector<uint8_t>& pchMessage, size_t dwLength) 
    {
        uint16_t wCRC = CRC_INIT;
        for (size_t i = 0; i < dwLength; ++i) 
        {
            wCRC = (wCRC >> 8) ^ wCRC_Table[(wCRC ^ pchMessage[i]) & 0xFF];
        }
        return wCRC;
    }

    std::tuple<std::vector<uint8_t>, std::vector<uint8_t>,uint8_t> 
    receive_packet(const std::vector<uint8_t>& data, const std::vector<uint8_t>& cmd_id, bool info=false)
    {
        // 查找SOF
        auto sof_iter = std::find(data.begin(), data.end(), static_cast<uint8_t>(0xA5));
        if (sof_iter == data.end())
        {
            if (info)
            {
                std::cout << "找不到SOF" << std::endl;
                return {};
            }
            
        }
        size_t sof_index = std::distance(data.begin(), sof_iter);
        // 检查长度
        if (data.size() < sof_index +5)
        {
            if(info)
            {
                std::cout << "数据不足(帧头)" << std::endl;
            }
            return {};
        }
        // 检查数据
        std::vector<uint8_t> frame_header(data.begin()+sof_index, data.begin() + sof_index +5);

        uint16_t data_length = frame_header[1] | (frame_header[2] << 8);
        if(data.size() <  sof_index + 5 + data_length + 2)
        {
            if(info)
            {
                std::cout << "数据不足(数据)" << std::endl;
            }
            return {};
        }
        
        uint8_t seq = frame_header[3];
        //CRC8校验
        if (Get_CRC8_Check_Sum({frame_header.begin(), frame_header.end() - 1}, 4) != frame_header[4])
        {
            if(info)
            {
                std::cout << "CRC8校验失败" << std::endl;
                return {};
            }
            
        }
        // 检查命令码
        std::vector<uint8_t> cmd_id_bytes(data.begin() + sof_index +5, data.begin() + sof_index +7);
        std::vector<uint8_t> cmd_id_le = {cmd_id[1], cmd_id[0]};
        if(cmd_id_bytes != cmd_id_le)
        {
            if(info)
            {
                std::cout << "命令码不匹配" << std::endl;
                return {};
            }
            
        }
        // 获取数据
        size_t data_start_index = sof_index + 5 + 2;
        size_t data_end_index = data_start_index + data_length;
        if (data_end_index > data.size())
        {
            if (info)
            {
                std::cout << "数据不足(data)" << std::endl;
            }
            return {};
        }
        std::vector<uint8_t> data_field(data.begin() + data_start_index, data.begin() + data_end_index);
        // 获取帧尾 
        size_t frame_tail_start = data_end_index;
        size_t frame_tail_end = frame_tail_start + 2;
        if(frame_tail_end > data.size())
        {
            if(info)
            {
                std::cout << "数据不足(帧尾)" << std::endl;
                return {};
            }
        }

        std::vector<uint8_t> frame_tail_bytes(data.begin() + frame_tail_start, data.begin() + frame_tail_end);
        uint16_t received_crc16 = frame_tail_bytes[0] | (frame_tail_bytes[1] << 8);

        // CRC16校验
        std::vector<uint8_t> crc16_input;
        crc16_input.insert(crc16_input.end(), frame_header.begin(), frame_header.end());
        crc16_input.insert(crc16_input.end(), cmd_id.begin(), cmd_id.end());
        crc16_input.insert(crc16_input.end(), data_field.begin(), data_field.end());

        uint16_t calculated_crc16 = Get_CRC16_Check_Sum(crc16_input, crc16_input.size());

        if(calculated_crc16 != received_crc16)
        {
            if(info)
            {
                std::cout << "CRC16校验失败" << std::endl;
                return {};
            }
        }
        return {cmd_id_bytes, data_field, seq};
    }

    RadarDecision radar_decision(uint8_t data)
    {
        if(typeid(data)==typeid(int))
        {
            std::cout<<"输入的必须是一个字节(0-255)"<<endl;
        }

        uint8_t double_vulnerability_chance = data & 0b00000011;
        uint8_t opponent_double_vulnerability = (data & 0b00000100) >> 2;
        uint8_t reserved_bits = (data & 0b11111000) >> 3;
    
        

        return {double_vulnerability_chance, opponent_double_vulnerability,reserved_bits};
        
    }

    void append_uint16_t_le(std::vector<uint8_t>& data, uint16_t value)
    {
        data.push_back(static_cast<uint8_t>(value & 0xFF));
        data.push_back(static_cast<uint8_t>((value >> 8) & 0xFF));
    }

    std::vector<uint8_t> build_data_radar_all(const std::unordered_map<std::string, std::pair<float,float>>& send_map, char color)
    {
        std::vector<uint8_t> data;
        if (color == 'R')
        {
            const char* keys[] = {"B1", "B2", "B3", "B4", "B5", "B7"};
            for (const auto& key : keys)
            {
                append_uint16_t_le(data, static_cast<uint16_t>(send_map.at(key).first));
                append_uint16_t_le(data, static_cast<uint16_t>(send_map.at(key).second));
            }
        }
        else
        {
            const char* keys[] = {"R1", "R2", "R3", "R4", "R5", "R7"};
            for (const auto& key : keys)
            {
                append_uint16_t_le(data, static_cast<uint16_t>(send_map.at(key).first));
                append_uint16_t_le(data, static_cast<uint16_t>(send_map.at(key).second));
            }
        }
        return data;
    }



    std::pair<std::vector<uint8_t>, uint8_t> build_send_packet(const std::vector<uint8_t>& data, const std::vector<uint8_t>& cmd_id, uint8_t& seq)
    {
        uint16_t data_length = (data.size());
        std::vector<uint8_t> frame_header;
        frame_header.push_back(0xA5);

        append_uint16_le(frame_header, data_length);

        frame_header.push_back(seq);

        uint8_t crc8 = Get_CRC8_Check_Sum(frame_header, 4);
        frame_header.push_back(crc8);

        std::vector<uint8_t> cmd_id_le = {cmd_id[1], cmd_id[0]};

        std::vector<uint8_t> packet(frame_header);
        packet.insert(packet.end(), cmd_id_le.begin(), cmd_id_le.end());
        packet.insert(packet.end(), data.begin(), data.end());

        uint16_t crc16 = Get_CRC16_Check_Sum(packet, packet.size());
        append_uint16_le(packet, crc16);

        return {packet, static_cast<uint8_t>(seq + 1) % 256};
    
    }

    void append_uint16_le(std::vector<uint8_t>& data, uint16_t value)
    {
        data.push_back(static_cast<uint8_t>(value & 0xFF));
        data.push_back(static_cast<uint8_t>((value >> 8) & 0xFF));
    }

    std::vector<uint8_t> build_data_decision(uint8_t chances, char &color)
    {
        std::vector<uint8_t> data;
        data.push_back(0x21);
        data.push_back(0x01);

        if(color == 'R')
        {
            append_uint16_le(data, 9);
        }
        else 
        {
            append_uint16_le(data, 109);
        }

        data.push_back(0x80);
        data.push_back(0x80);

        data.push_back(chances);
        return data;
       
        
    }

        

//};

//class SerialManager : public SerialPort
//    {
//        public:

            SerialManager(const std::string& port, int bandrate = 115200, char color = 'R'):
            port_(port), bandrate_(bandrate), color_(color), seq_(0)
            {
                last_send_time_ = std::chrono::steady_clock::now() - std::chrono::seconds(11);
            }
            
        bool serial_set()
        {
            ser.setPort(port_);
            ser.setBaudrate(bandrate_);
            serial::Timeout timeout = serial::Timeout::simpleTimeout(1000);
            ser.setTimeout(timeout);
            ser.open();
            if (!ser.isOpen())
            {
                std::cout << "串口设置失败" << std::endl;
                return false;
            }
        }
        void receive_serial() 
        {
            std::vector<uint8_t> buffer;
            std::vector<uint8_t> progress_cmd_id = {0x0C, 0x02};
            std::vector<uint8_t> vulnerability_cmd_id = {0x0E, 0x01};
            while (true)
            {
             std::vector<uint8_t> receiver_data = serial_read_all();
             buffer.insert(buffer.end(), receiver_data.begin(), receiver_data.end());
             
             auto sof_iter = std::find(buffer.begin(), buffer.end(), 0xA5);
             while (sof_iter != buffer.end())
             {
                if (buffer.end() - sof_iter < 5) break;
                size_t sof_index = std::distance(buffer.begin(), sof_iter);
                std::vector<uint8_t> packet_data(buffer.begin() + sof_index, buffer.end());
                auto next_sof = std::find(packet_data.begin(), packet_data.end(), 0xA5);
                if (next_sof != packet_data.end())
                {
                    packet_data.erase(next_sof, packet_data.end());
                }
                else
                {
                    break;
                }

                auto progress_result = receive_packet(packet_data, progress_cmd_id, false);
                auto vulnerability_result = receive_packet(packet_data, vulnerability_cmd_id, false);

                if (std::get<1>(progress_result).size() >0)
                {
                    std::lock_guard<std::mutex> lock(mutex_);
                    progress_list.clear();
                    for (auto v : std::get<1>(progress_result)) progress_list.push_back(v);
                    if (color_ == 'R')
                    {
                        mark_value["B1"] = progress_list[0];
                        mark_value["B2"] = progress_list[1];
                        mark_value["B7"] = progress_list[5];
                    }
                    else
                    {
                        mark_value["R1"] = progress_list[0];
                        mark_value["R2"] = progress_list[1];
                        mark_value["R7"] = progress_list[5];
                    }

                }

                if (std::get<1>(vulnerability_result).size() >0)

                {
                    auto res = radar_decision(std::get<1>(vulnerability_result)[0]);
                    double_vulnerability_chance = res.double_vulnerability_chance;
                    opponent_double_vulnerability = res.opponent_double_vulnerability;
                    
                }

                buffer.erase(buffer.begin(), buffer.begin() + sof_index + packet_data.size());
                sof_iter = std::find(buffer.begin(), buffer.end(), 0xA5);
                }
                std::this_thread::sleep_for(std::chrono::milliseconds(500));
            }

        }
        void send_serial(const std::unordered_map<std::string, std::pair<float,float>>* position = nullptr)
        {
            std::unordered_map<std::string, std::pair<int,int>> send_map =
            {
                {"R1",{0,0}}, {"R2",{0,0}}, {"R3",{0,0}}, {"R4",{0,0}}, {"R5",{0,0}}, {"R6",{0,0}}, {"R7",{0,0}},
                {"B1",{0,0}}, {"B2",{0,0}}, {"B3",{0,0}}, {"B4",{0,0}}, {"B5",{0,0}}, {"B6",{0,0}}, {"B7",{0,0}}
            };
            if (position)
            {
                for (const auto& [robot_id, pos] : *position)
                {
                    int x = static_cast<int>(pos.first * 100 +0.5f);
                    int y = static_cast<int>(pos.second * 100 +0.5f);
                    if (send_map.count(robot_id))
                    {
                        if (color_ == 'R')
                        {
                            send_map[robot_id] = {x, 1500 - y};
                        }
                        else
                        {
                            send_map[robot_id] = {x, y};
                        }
                    }
                }
            }

            try 
            {
                auto ser_data = build_data_radar_all(send_map, color_);
                auto [packet, next_seq] = build_send_packet(ser_data, {0x03, 0x05}, seq_);
                seq_ = next_seq;
                size_t bytes_writen = ser.write(packet);
                std::cout << "数据包:" << bytes_writen << std::endl;
                auto now = std::chrono::steady_clock::now();
                if (std::chrono::duration_cast<std::chrono::seconds>(now - last_send_time_).count() >= 10)
                {
                    auto data = build_data_decision(chances_flag, color_);
                    auto [packet, next_seq] = build_send_packet(data, {0x03, 0x05}, seq_);
                    seq_ = next_seq;
                    size_t chance_writen = ser.write(packet);
                    std::cout << chance_writen << std::endl;
                    std::cout<<"双倍易伤触发成功, 标志位: " << chances_flag << std::endl;
                    chances_flag++;
                    if (chances_flag >= 3) chances_flag = 1;
                    last_send_time_ = now;
                }
            }
            catch (const std::exception& e)
            {
                std::cerr << "出现错误: "<< e.what() << std::endl;
            }
        }

        void stop()
        {
            ser.close();
            std::cout << "串口关闭成功" << std::endl;
        }

        //private:
        //serial::Serial ser;
        //std::string port_;
        //int bandrate_;
        //char color_;
        //uint8_t seq_;
        //std::chrono::steady_clock::time_point last_send_time_;
        //std::mutex mutex_;


        std::vector<uint8_t> serial_read_all()
        {
            return {};
        }
        //void serial_write(const std::vector<uint8_t>& data)
        //{
            
        //}
    //};

//int main()
//{
    
//}


////////改全局

// 解析单个数据包
// 帧头frame_header格式：SOF(1-byte) + data_length(2-byte) + seq(1-byte) + CRC8(1-byte)
// 整个协议格式: frame_header(5-byte) + cmd_id(2-byte) + data(n-byte) + frame_tail(2-byte, CRC16)
// data是需要处理的数据包；cmd_id是相应的序列号; info是指要不要输出日志，一般选True\False
    


    

   

    







    












//opponent_double_vulnerabillity
//opponent_double_vulnerability