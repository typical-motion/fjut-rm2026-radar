...
uint8_t SerialPort::Get_CRC8_Check_Sum(const std::vector<uint8_t>& pchMessage, size_t dwLength)
{
    ...
    for (size_t i = 0; i < dwLength && i < pchMessage.size(); ++i)
    {
...
SerialPort::receive_packet(const std::vector<uint8_t>& data, const std::vector<uint8_t>& cmd_id, bool info)
{
    ...
    {
        if (info) std::cout << "找不到SOF" << std::endl;
        return {{}, {}, 0};
    }
    ...
    {
        if (info) std::cout << "数据不足(帧头)" << std::endl;
        return {{}, {}, 0};
    }
    ...
    // 检查整体长度（frame_header + cmd_id(2) + data(data_length) + frame_tail(2)）
    if (data.size() < sof_index + 5 + 2 + data_length + 2)
    {
        if (info) std::cout << "数据不足(数据)" << std::endl;
        return {{}, {}, 0};
    }
    ...
    {
        if (info) std::cout << "CRC8校验失败" << std::endl;
        return {{}, {}, 0};
    }
    ...
    {
        if (info) std::cout << "命令码不匹配" << std::endl;
        return {{}, {}, 0};
    }
    ...
    {
        if (info) std::cout << "数据不足(data)" << std::endl;
        return {{}, {}, 0};
    }
    ...
    {
        if (info) std::cout << "数据不足(帧尾)" << std::endl;
        return {{}, {}, 0};
    }
    std::vector<uint8_t> frame_tail_bytes(data.begin() + frame_tail_start, data.begin() + frame_tail_end);
    uint16_t received_crc16 = static_cast<uint16_t>(frame_tail_bytes[0]) | (static_cast<uint16_t>(frame_tail_bytes[1]) << 8);
    ...
    {
        if (info) std::cout << "CRC16校验失败" << std::endl;
        return {{}, {}, 0};
    }
...
std::vector<uint8_t> SerialPort::build_data_radar_all(const std::unordered_map<std::string, std::pair<int,int>>& send_map, char color)
{
...
std::pair<std::vector<uint8_t>, uint8_t> SerialPort::build_send_packet(const std::vector<uint8_t>& data, const std::vector<uint8_t>& cmd_id, uint8_t& seq)
{
    ...
    frame_header.push_back(0xA5);//add SOF

    append_uint16_le(frame_header, data_length);//add data_length

    frame_header.push_back(seq);//add seq

    uint8_t crc8 = Get_CRC8_Check_Sum(frame_header, 4);
    frame_header.push_back(crc8);//add crc8
...
{
    try { if (ser.isOpen()) ser.close(); } catch (...) {}
}
...
void SerialManager::receive_serial()
{
    ...
    while (true)
    {
        ...
        while (sof_iter != buffer.end())
        {
            ...
            // 查找下一个 SOF（从下一个字节开始）
            auto next_sof = std::find(packet_data.begin() + 1, packet_data.end(), 0xA5);
            ...
            {
                // 如果没有找到下一个 SOF，说明当前 buffer 可能还不完整，等待更多数据
                break;
            }
            ...
            if (std::get<1>(progress_result).size() > 0)
            {
                ...
                if (color_ == 'R')
                {
                    if (progress_list.size() > 0) mark_value["B1"] = progress_list.size() > 0 ? progress_list[0] : 0;
...
void SerialManager::send_serial(const std::unordered_map<std::string, std::pair<float,float>>* position)
{
    ...
    try
    {
        ...
        std::cout << "数据包: " << bytes_written << std::endl;
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_send_time_).count() >= 10)
        {
            ...
            std::cout << "触发包写入字节数: " << chance_written << std::endl;
...
void SerialManager::stop()
{
    ...
    {
        std::cerr << "关闭串口异常: " << e.what() << std::endl;
    }
